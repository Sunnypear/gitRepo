...
因此，您可以传递不同大小的图像，并使用适当的原始点坐标调用绘图例程。
    ...
    def save_to_img(self,sequence_dir,filename):
        cv2.imwrite(os.path.join(sequence_dir,filename),self.image)
    ...
    def circle(self, x, y, radius, label=None):
        ...
        center = image.shape[1] // 2, image.shape[0] // 2
        cv2.circle(
            image, center, int(radius + 1.5), self._color, self.thickness)
        if label is not None:
            cv2.putText(
