...
#from tensorflow.contrib.slim import arg_scope 注释掉原代码，改从tf_slim 中调用arg_scope
...
def _preprocess(image):
    """
    在做图像处理时，一张RGB图片一般有3个维度、3个通道。
    image[:,:,::-1]作用：对颜色通道做变换，将图片从RGB图片转成BGR图片
    image[:, ::-1, :]作用：将图像进行左右翻转
    image[::-1, :, :]作用：将图像上下颠倒
    """
...
def main():
    ...
    with tf.compat.v1.Session(graph=tf.Graph()) as session:
        ...
        # tf>cast(x,dtype,name=None)，将x的类型装换为dtype
        image_var = tf.map_fn(
