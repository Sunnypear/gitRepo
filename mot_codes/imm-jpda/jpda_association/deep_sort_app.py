...
def run(sequence_dir, detection_file, output_file, min_confidence,
    ...
    def frame_callback(vis, frame_idx):
        ...
        next_frame_indices = [next_frame_detections[i] for i in next_frame_indices]
        ...
        if len(detections) == 0:
            return
        # 对现有的所有轨迹进行预测处理，得到每个轨迹关联门内的检测索引
        tracker.start_new_track(detections)
        tracker.predict()
        # 计算关联概率
        jpda_prob_matrix = tracker.cal_jpda_prob(detections,frame_idx,threshold=0.0)
        tracker.update(jpda_prob_matrix,detections,next_frame_detections,frame_idx)
