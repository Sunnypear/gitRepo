...
class Track:
    ...
    def __init__(self,frame_id,mean, covariance, sita,imm_controller,track_id, n_init, max_age,
                 curdetection_id,curdetection_mean,curdetection_frameid,feature=None, detect_prob=0.9):
        ...
        # 记录当前时刻，关联到轨迹的量测的id和位置信息
        self.curdetection_id = curdetection_id
        self.curdetection_mean = curdetection_mean
        self.curdetection_frameid = curdetection_frameid
    ...
    def find_detections_in_association_Doors_by_iou(self,detections,frame_idx,threshold=0.0):
        measure_index_list, measure_distance_list = self.imm_controller.find_detections_in_association_Doors_by_iou(\
            detections,self.sita,frame_idx)
        measure_index_list, measure_distance_list, model_ind= self.consolidate_same_measure_distance(\
            measure_index_list, measure_distance_list)
        # 对多模型进行剪枝后的检测列表
    ...
    def update_by_jpda(self,jpda_prob_matrix,detections,track_idx,detection_idx,frame_idx):
        ...
        self.curdetection_mean = detections[detection_idx]
        self.curdetection_id = detection_idx
        self.curdetection_frameid = frame_idx
        self.hits += 1
        self.time_since_update = 0
        if self.state == TrackState.Tentative and self.hits >= self._n_init:
            self.state = TrackState.Confirmed

    def update_by_kalman(self,kf, detection,detection_idx,frame_idx):
        # cur_mean = self.mean.copy()
        # cur_bxy = cur_mean[0:2]
        # cur_bxy[1] = cur_mean[2] + (cur_mean[6]/2.)
        # self.sita = self.math_operation.get_track_direction_sita(detection.to_bcxywh()[0:2],cur_bxy)
        # self.mean, self.covariance = self.imm_controller.update_by_kalman(detection)
        last_mean = self.mean.copy()
        last_bxy = last_mean[0:2]
        # last_bxy[1] = last_mean[2] + (last_mean[6] / 2.)
        # self.sita = self.math_operation.get_track_direction_sita(detection.to_bcxywh()[0:2],cur_bxy)
        # # self.mean, self.covariance = kf.update(
        ...
        cur_bxy = cur_mean[0:2]
        # cur_bxy[1] = cur_mean[2] + (cur_mean[6] / 2.)
        self.sita = (self.sita + self.math_operation.get_track_direction_sita(cur_bxy, last_bxy)) / 2
        ...
        self.curdetection_mean = detection
        self.curdetection_id = detection_idx
        self.curdetection_frameid = frame_idx
