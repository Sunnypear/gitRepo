...
class Tracker:
    ...
    # 调用的时候，后边的参数全部是默认的
    def __init__(self, metric, max_iou_distance=0.7, max_age=10, n_init=3,dynamic_model_list=['constant_velocity'],model_max_velocity=[5.0,5.0,1.0,1.0]):
        ...
        # 直到当前帧结束的轨迹合集，应该在每一帧开始前进行比较，轨迹在结束之前必定参与比较；
        # 当一个轨迹结束要加入该集合，与此集合中的检测进行合并
        # 每个轨迹，只记录（轨迹号，开始帧、结束帧）
        self.until_cur_frame_end_tracks = {}
        # 记录轨迹号，只记录（轨迹号，开始帧）,如果到当前帧，轨迹长度大于10，才开始合并
        self.until_cur_frame_runing_tracks = {}
        # associated_frames_door是新开始轨迹的开始帧与已经结束轨迹的结束帧之间的间隔，在这个间隔内的轨迹才进行匹配
        self.associated_frames_door = 10
    ...
    def _initiate_track(self, new_tracks_start_frame_id,detectionstart_id, detection_start, detectionend_frameidx,detectionend_id,detection_end):
        dmean = detection_end.to_cxy()-detection_start.to_cxy()
        sita = self.math_operation.get_track_direction_sita(detection_end.to_cxy()[:2],detection_start.to_cxy()[:2])
        ...
        mean, covariance = imm_controller.initiate_dynamic_model(detection_end.to_cxy(),dmean)
        ...
        # 记录轨迹号，只记录（轨迹号，开始帧）,如果到当前帧，轨迹长度大于10，才开始合并
        # 在轨迹初始化时，将轨迹加入self.until_cur_frame_runing_tracks = []
        self.add_track_into_until_cur_frame_runing_tracks(self._next_id,[new_tracks_start_frame_id])
        # self.until_cur_frame_runing_tracks["{}".format(self._next_id)]=[new_tracks_start_frame_id]
    ...
    def _nn_euclidean_distance(self, x, y, threshold_distance=6,threshold_aera=8):
        # 计算余弦距离
        y_cxy = [det.to_cxy() for det in y]
        # y_wh = [det.tlwh[2:4] for det in y]
        distances = self.pdist([x.to_cxy()], y_cxy, threshold_distance)
        # area = self.aera_dist([x.tlwh[2:4]], y_wh, threshold_aera)
        # total_distance = distances + area
    ...
    def get_cost_matrix(self,detections,next_frame_detections):
        ...
        for i, detection in enumerate(detections):
            cost_matrix[i, :] = self._nn_euclidean_distance(detections[i], next_frame_detections)
        return cost_matrix

    def start_tracks(self,detections,frame_idx,threshold=100):
        # 为第一帧初始化轨迹
        if len(detections)==0:
    ...
    def initiate_a_new_track(self, detectionid,detection,next_frame_detectionid,next_frame_detection,frame_id):
        dmean = next_frame_detection.to_cxy()-detection.to_cxy()
        sita = self.math_operation.get_track_direction_sita(next_frame_detection.to_bcxy(),detection.to_bcxy())
        imm_controller = Imm_Controller(self.dynamic_model_list,self.model_max_velocity,\
                                                       deviation_process_noise = 0.5,deviation_measurement_noise=7,\
                                                       dynamic_model_transition_pro_matrix=[[1.0]])
        # 这里初始化时传入的是下一帧检测，因为已经实现了下一帧检测与当前帧检测的分配，因此下一帧的检测才未当前轨迹的最新状态位置
        mean, covariance = imm_controller.initiate_dynamic_model(next_frame_detection.to_cxy(),dmean)
    ...
    def add_track_into_until_cur_frame_end_tracks(self, trackid, frame_pair):
        self.until_cur_frame_end_tracks["{}".format(trackid)] = frame_pair


    def consolidate_track_in_until_cur_frame_end_tracks(self):
        if len(self.until_cur_frame_end_tracks.keys()) <2:
            return
        # 当向until_cur_frame_end_tracks添加轨迹时，需要先进行一次合并
        until_cur_frame_end_tracks_keys = self.until_cur_frame_end_tracks.keys()
        sorted_until_cur_frame_end_tracks_keys = sorted(until_cur_frame_end_tracks_keys, key=lambda k: int(k))
        #记录已经移除的
        trackid_removed = []
        for a_track_i in range(len(sorted_until_cur_frame_end_tracks_keys)):
            a_track_i_id = sorted_until_cur_frame_end_tracks_keys[a_track_i]
            same_tracks = []
            if a_track_i_id in trackid_removed:
                continue
            for a_track_j in range(a_track_i+1,len(sorted_until_cur_frame_end_tracks_keys)):
                a_track_j_id = sorted_until_cur_frame_end_tracks_keys[a_track_j]
                if a_track_j_id in trackid_removed:
                    continue
                lab = True
                if len(self.tracks_record[a_track_i_id].keys()) == len(self.tracks_record[a_track_j_id].keys()):
                    sorted_track_i_keys = sorted(self.tracks_record[a_track_i_id].keys(), key=lambda k: int(k))
                    sorted_track_j_keys = sorted(self.tracks_record[a_track_j_id].keys(),key=lambda k: int(k))
                    for ii in range(len(sorted_track_i_keys)):
                        if sorted_track_i_keys[ii] == sorted_track_j_keys[ii]:
                            if self.tracks_record[a_track_i_id][sorted_track_i_keys[ii]].keys() != \
                                    self.tracks_record[a_track_j_id][sorted_track_j_keys[ii]].keys():
                                lab = False
                        else:
                            lab = False
                else:
                    lab = False
                if lab:
                    same_tracks.append(a_track_j_id)
            if len(same_tracks) !=0:
                same_tracks.append(a_track_i_id)
                int_same_tracks = [int(a) for a in same_tracks]
                # 保留轨迹号小的那个轨迹
                ind = int_same_tracks.index(min(int_same_tracks))
                reserved_trackid = same_tracks[ind]
                for a_trackid in same_tracks:
                    if a_trackid != reserved_trackid:
                        trackid_removed.append(a_trackid)
                        self.until_cur_frame_end_tracks.pop(a_trackid)
                        self.tracks_record.pop(a_trackid)
                        self.tracks_state.pop(a_trackid)
                        self.update_detection_record(reserved_trackid, a_trackid)
        until_cur_frame_end_tracks_keys = self.until_cur_frame_end_tracks.keys()
        sorted_until_cur_frame_end_tracks_keys = sorted(until_cur_frame_end_tracks_keys, key=lambda k: int(k))
        # 记录已经移除的
        trackid_removed = []

        for end_ind_i in range(len(sorted_until_cur_frame_end_tracks_keys)):
            end_track_id_i = sorted_until_cur_frame_end_tracks_keys[end_ind_i]
            if end_track_id_i in trackid_removed or len(self.tracks_record[end_track_id_i].keys()) <2:
                # 这是可能的，不处理
                continue
            else:
                cost_matrix = 180.*np.ones((len(sorted_until_cur_frame_end_tracks_keys),1))
                start_end_frame_i= self.until_cur_frame_end_tracks[end_track_id_i]
                track_serial_i = self.tracks_record[end_track_id_i]
                # 取出轨迹中的帧排序索引
                track_frame_sorted_index_i = sorted(track_serial_i.keys(), key=lambda k: int(k))
                serial_i_inds = track_frame_sorted_index_i[0:] if len(track_frame_sorted_index_i) < 10 else \
                    track_frame_sorted_index_i[len(track_frame_sorted_index_i) - 10:len(track_frame_sorted_index_i)]
                # inds = track_frame_sorted_index[self.until_cur_frame_end_tracks[end_track_id][0]-1:self.until_cur_frame_end_tracks[end_track_id][1]]
                track_frames_i = []
                for idx in serial_i_inds:
                    a_dict = self.tracks_record[end_track_id_i][idx]
                    keyss = list(a_dict.keys())
                    track_frames_i.append(self.tracks_record[end_track_id_i][idx][keyss[0]])
                start_posi_i = track_frames_i[0].to_cxy()
                end_posi_i = track_frames_i[-1].to_cxy()
                for end_ind_j in range(len(sorted_until_cur_frame_end_tracks_keys)):
                    end_track_id_j = sorted_until_cur_frame_end_tracks_keys[end_ind_j]
                    # end_track_id_j == end_track_id_i,不和自己匹配
                    if end_track_id_j in trackid_removed or len(self.tracks_record[end_track_id_j].keys()) < 2 or end_track_id_j == end_track_id_i:
                        # 这是可能的，不处理
                        continue
                    track_serial_j = self.tracks_record[end_track_id_j]
                    start_end_frame_j = self.until_cur_frame_end_tracks[end_track_id_j]
                    #进行轨迹合并检查
                    # 判断当前轨迹i是否时间上可以成为j的后段轨迹
                    if (start_end_frame_i[0]-start_end_frame_j[1]) >0 and\
                            (start_end_frame_i[0]-start_end_frame_j[1]) <self.associated_frames_door:
                        # 取出轨迹中的帧排序索引
                        track_frame_sorted_index_j = sorted(track_serial_j.keys(), key=lambda k: int(k))
                        serial_j_inds = track_frame_sorted_index_j[0:] if len(track_frame_sorted_index_j) < 10 else \
                            track_frame_sorted_index_j[
                            len(track_frame_sorted_index_j) - 10:len(track_frame_sorted_index_j)]
                        # inds = track_frame_sorted_index[self.until_cur_frame_end_tracks[end_track_id][0]-1:self.until_cur_frame_end_tracks[end_track_id][1]]
                        track_frames_j = []
                        for idx in serial_j_inds:
                            a_dict = self.tracks_record[end_track_id_j][idx]
                            keyss = list(a_dict.keys())
                            track_frames_j.append(self.tracks_record[end_track_id_j][idx][keyss[0]])
                        start_posi_j = track_frames_j[0].to_cxy()
                        end_posi_j = track_frames_j[-1].to_cxy()
                        # 这里取得最大速度，dx,dy
                        max_dx, max_dy = self.math_operation.get_dxdy([det.to_cxy() for det in track_frames_j])
                        # 这里取得帧数差，由帧数差和最大速度，dx,dy，得到最大的关联域
                        interval_frame_num = start_end_frame_i[0] - start_end_frame_j[1]
                        max_posi = [interval_frame_num * max_dx, interval_frame_num * max_dy]
                        max_distance = self.math_operation.cal_distance_bet_points([0., 0.], max_posi)
                        if self.math_operation.is_in_posi1_and_posi2(start_posi_i[0], end_posi_j[0], \
                                                                 end_posi_j[0] + interval_frame_num * max_dx) and \
                                self.math_operation.is_in_posi1_and_posi2(start_posi_i[1], end_posi_j[1], \
                                                                      end_posi_j[1] + interval_frame_num * max_dy):
                            cur_distance = self.math_operation.cal_distance_bet_points(end_posi_j,end_posi_i)
                            match_rate = self.match_bet_tracks(track_frames_i, [track_frames_j])

                            cost_matrix[end_ind_j, 0] = 0.2*interval_frame_num/self.associated_frames_door +\
                                                        0.3*cur_distance/max_distance+0.5*match_rate[0]/180.
                        else:
                            cost_matrix[end_ind_j, 0] = 180.
                    else:
                        cost_matrix[end_ind_j, 0] = 180.
                # 至此，得到cost_matrix矩阵，进行分配
                ind = cost_matrix[:,0].argmin()
                if cost_matrix[ind,0] != 180. and cost_matrix[ind,0] <(0.2*5/self.associated_frames_door +\
                                                            0.3+0.5*30/180.):
                    # 完成匹配
                    matched_head_trackid = sorted_until_cur_frame_end_tracks_keys[ind]
                    track_id_trans_pair = [int(matched_head_trackid),int(end_track_id_i)]
                    head_tracks_serial = self.tracks_record[matched_head_trackid]
                    tail_tracks_serial = self.tracks_record[end_track_id_i]
                    temp_track = {}
                    temp_track[matched_head_trackid] = {}
                    for head_ind in head_tracks_serial.keys():
                        temp_track[matched_head_trackid][head_ind] = head_tracks_serial[head_ind]
                    for tail_ind in tail_tracks_serial.keys():
                        temp_track[matched_head_trackid][tail_ind] = tail_tracks_serial[tail_ind]
                    self.tracks_record[matched_head_trackid] = temp_track[matched_head_trackid]
                    trackid_removed.append(end_track_id_i)
                    self.tracks_record.pop(end_track_id_i)
                    self.tracks_state.pop(end_track_id_i)
                    self.until_cur_frame_end_tracks[matched_head_trackid] = [self.until_cur_frame_end_tracks[matched_head_trackid][0],start_end_frame_i[1]]
                    self.until_cur_frame_end_tracks.pop(end_track_id_i)
                    self.update_detection_record(matched_head_trackid,end_track_id_i)

    def add_track_into_until_cur_frame_runing_tracks(self,trackid,frame_pair):
        self.until_cur_frame_runing_tracks["{}".format(trackid)] = frame_pair
    def match(self, detections, undealed_detections, next_frame_detections, frame_idx):
        ...
        for tra in self.tracks:
            if tra.is_deleted():
                ...
                if "{}".format(tra.track_id) in self.until_cur_frame_runing_tracks.keys():
                    start_frame = self.until_cur_frame_runing_tracks.pop("{}".format(tra.track_id))
                self.add_track_into_until_cur_frame_end_tracks(tra.track_id,[start_frame[0],tra.curdetection_frameid])
                # self.until_cur_frame_end_tracks["{}".format(tra.track_id)] = [start_frame[0],frame_idx]
            else:
    ...
    def update_detection_record(self,cur_track,origin_track,):
        detection_dict_sorted_index = sorted(self.detection_record.keys(), key=lambda k: int(k))
        for update_frame_ind in detection_dict_sorted_index:
            for a_detection in self.detection_record[update_frame_ind].keys():
                if origin_track in self.detection_record[update_frame_ind][a_detection]:
                    flag = 0
                    if cur_track in self.detection_record[update_frame_ind][a_detection]:
                        flag = 1
                    ind = self.detection_record[update_frame_ind][a_detection].index(origin_track)
                    self.detection_record[update_frame_ind][a_detection][ind] = cur_track
                    if flag == 1:
                        lab = 0
                        for tr in self.detection_record[update_frame_ind][a_detection]:
                            if tr == cur_track:
                                lab += 1
                        if len(self.detection_record[update_frame_ind][a_detection]) == lab:
                            self.detection_record[update_frame_ind][a_detection] = [cur_track]
    ...
    def consolidate_tracks(self,frame_idx,wait_frames,associated_frames_door):
        cost_matrix = 180.*np.ones((len(self.until_cur_frame_end_tracks.keys()),len(self.until_cur_frame_runing_tracks.keys())))
        until_cur_frame_end_tracks_keys = self.until_cur_frame_end_tracks.keys()
        sorted_until_cur_frame_end_tracks_keys = sorted(until_cur_frame_end_tracks_keys, key=lambda k: int(k))
        for end_ind in range(len(sorted_until_cur_frame_end_tracks_keys)):
            # 取出对应轨迹
            end_track_id = sorted_until_cur_frame_end_tracks_keys[end_ind]
            track_serial = self.tracks_record[end_track_id]
            if len(track_serial.keys())<2:
                continue
            # 取出轨迹中的帧排序索引
            track_frame_sorted_index = sorted(track_serial.keys(), key=lambda k: int(k))
            inds = track_frame_sorted_index[0:] if len(track_frame_sorted_index) < 10 else track_frame_sorted_index[len(track_frame_sorted_index)-10:len(track_frame_sorted_index)]
            # inds = track_frame_sorted_index[self.until_cur_frame_end_tracks[end_track_id][0]-1:self.until_cur_frame_end_tracks[end_track_id][1]]
            track_frames = []
            for idx in inds:
                a_dict = self.tracks_record[end_track_id][idx]
                keyss = list(a_dict.keys())
                track_frames.append(self.tracks_record[end_track_id][idx][keyss[0]])
            # 取出新开始轨迹
            until_cur_frame_runing_tracks_keys = self.until_cur_frame_runing_tracks.keys()
            sorted_until_cur_frame_runing_tracks_keys = sorted(until_cur_frame_runing_tracks_keys, key=lambda k: int(k))
            for runing_ind in range(len(sorted_until_cur_frame_runing_tracks_keys)):
                runing_track_id = sorted_until_cur_frame_runing_tracks_keys[runing_ind]
                # 判断结束轨迹的结束帧号与新开始轨迹的开始帧号是否满足关联门
                # 只有新开始轨迹的开始帧号大于结束轨迹的结束帧号，且新开始轨迹的开始帧号与结束轨迹的结束帧号在预设的帧间隔内，才进行匹配
                # 比如只匹配在10帧内的轨迹
                if (self.until_cur_frame_runing_tracks[runing_track_id][0]-self.until_cur_frame_end_tracks[end_track_id][1]) >0 and\
                        (self.until_cur_frame_runing_tracks[runing_track_id][0]-self.until_cur_frame_end_tracks[end_track_id][1]) <associated_frames_door:
                    # 判断当前帧时，当前正在运行轨迹的长度是否满足要求,只能轨迹达到一定长度，才能提取足够特征
                    if frame_idx - self.until_cur_frame_runing_tracks[runing_track_id][0] >= wait_frames:
                        # 只有需要根据轨迹在场景中的的实际位置设定关联门进行关联
                        cur_end_track_posi = track_frames[-1].to_cxy()
                        new_track = self.tracks_record[runing_track_id]
                        new_track_sorted_index = sorted(new_track.keys(), key=lambda k: int(k))
                        inds = new_track_sorted_index if len(new_track_sorted_index) <20 else new_track_sorted_index[0:20]
                        new_tracks = []
                        for idx in inds:
                            a_frame_dict = self.tracks_record[runing_track_id][idx]
                            det_keys = list(a_frame_dict.keys())
                            new_tracks.append(a_frame_dict[det_keys[0]])
                        cur_new_track_posi = new_tracks[0].to_cxy()
                        # 这里取得最大速度，dx,dy
                        max_dx,max_dy = self.math_operation.get_dxdy([det.to_cxy() for det in track_frames])
                        # 这里取得帧数差，由帧数差和最大速度，dx,dy，得到最大的关联域
                        interval_frame_num = self.until_cur_frame_runing_tracks[runing_track_id][0] - self.until_cur_frame_end_tracks[end_track_id][1]
                        if self.math_operation.is_in_posi1_and_posi2(cur_new_track_posi[0],cur_end_track_posi[0],\
                                                                     cur_end_track_posi[0]+interval_frame_num*max_dx) and\
                                self.math_operation.is_in_posi1_and_posi2(cur_new_track_posi[1], cur_end_track_posi[1],\
                                                                  cur_end_track_posi[1] + interval_frame_num * max_dy):
                            max_posi = [interval_frame_num * max_dx, interval_frame_num * max_dy]
                            max_distance = self.math_operation.cal_distance_bet_points([0., 0.], max_posi)
                            cur_distance = self.math_operation.cal_distance_bet_points(cur_end_track_posi,cur_new_track_posi)
                            match_rate = self.match_bet_tracks(track_frames,[new_tracks])
                            aa=max_distance
                            bb=cur_distance
                            cc=interval_frame_num
                            if max_distance == 0.0:
                                if  cur_distance <1.:
                                    cost_matrix[end_ind,runing_ind] = 0.2*interval_frame_num/self.associated_frames_door +\
                                                        0.3*cur_distance+0.5*match_rate[0]/180.
                                else:
                                    cost_matrix[end_ind, runing_ind] = 180.
                            else:
                                cost_matrix[end_ind, runing_ind] = 0.2 * interval_frame_num / self.associated_frames_door + \
                                                           0.3 * cur_distance / max_distance + 0.5 * match_rate[0] / 180.
                        else:
    ...
    def match_bet_new_and_end_track(self, frame_idx,wait_frames):
        cost_matrix = self.consolidate_tracks(frame_idx,wait_frames,self.associated_frames_door)
        until_cur_frame_end_tracks_keys = self.until_cur_frame_end_tracks.keys()
        sorted_until_cur_frame_end_tracks_keys = sorted(until_cur_frame_end_tracks_keys, key=lambda k: int(k))
        until_cur_frame_runing_tracks_keys = self.until_cur_frame_runing_tracks.keys()
        sorted_until_cur_frame_runing_tracks_keys = sorted(until_cur_frame_runing_tracks_keys, key=lambda k: int(k))
        ...
        #     if cost_matrix[row_t[i], col_t[i]] <30.:
        #         row_tt.append(row_t[i])
        #         col_tt.append(col_t[i])
        row_tt = np.array(row_t)
        col_tt = np.array(col_t)
        # indices_temp = np.c_[row_t[:, np.newaxis], col_t[:, np.newaxis]]
        indices = np.c_[row_tt[:, np.newaxis], col_tt[:, np.newaxis]]
        for row, col in indices:
            end_trackid = sorted_until_cur_frame_end_tracks_keys[row]
            new_trackid = sorted_until_cur_frame_runing_tracks_keys[col]
            if cost_matrix[row, col] != 180. and cost_matrix[row,col] <(0.2*5/self.associated_frames_door +\
                                                            0.3+0.5*30/180.):
                print("ssss",end_trackid,new_trackid)
                print(cost_matrix[row, col])
                matched_tracks.append([end_trackid, new_trackid])
        for head_trackid,tail_trackid in matched_tracks:
            head_track = self.tracks_record[head_trackid]
            sorted_head_tracks_keys = sorted(head_track.keys(), key=lambda k: int(k))
            tail_track = self.tracks_record[tail_trackid]
            sorted_tail_tracks_keys = sorted(tail_track.keys(), key=lambda k: int(k))
            temp_track = {}
            temp_track[head_trackid] = {}
            for key in sorted_head_tracks_keys:
                temp_track[head_trackid][key] = head_track[key]
            for key in sorted_tail_tracks_keys:
                temp_track[head_trackid][key] = tail_track[key]
            self.tracks_record[head_trackid] = temp_track[head_trackid]
            self.tracks_state[head_trackid] = self.tracks_state[tail_trackid]
            self.until_cur_frame_runing_tracks[head_trackid] = [self.until_cur_frame_end_tracks[head_trackid][0]]
            self.until_cur_frame_end_tracks.pop(head_trackid)
            self.until_cur_frame_runing_tracks.pop(tail_trackid)
            self.tracks_record.pop(tail_trackid)
            self.tracks_state.pop(tail_trackid)
            self.update_detection_record(int(head_trackid),int(tail_trackid))
            for track in self.tracks:
                if track.track_id == int(tail_trackid):
                    track.track_id = int(head_trackid)
    ...
    def match_by_iou(self, detections):
        ...
        for trackid,track in enumerate(self.tracks):
            ...
            if len(associated_detections_list)==0:
                ...
                # min_distance = min(associated_detections_distance)
                index = np.argmin(associated_detections_distance)
    ...
    def remove_fake_tracks(self,track_len):
        temp_track_recored = copy.deepcopy(self.tracks_record)
        temp_track_state = copy.deepcopy(self.tracks_state)
        temp_until_cur_frame_end_tracks = copy.deepcopy(self.until_cur_frame_end_tracks)
        for track_id in self.tracks_record.keys():
            if len(self.tracks_record[track_id].keys()) < track_len and  self.tracks_state[track_id] == True:
                temp_track_recored.pop(track_id)
                temp_track_state.pop(track_id)
                temp_until_cur_frame_end_tracks.pop(track_id)
        self.tracks_record = temp_track_recored
        self.tracks_state = temp_track_state
        self.until_cur_frame_end_tracks = temp_until_cur_frame_end_tracks
    ...
    def get_unnormal_points(self):
        ...
        for track_id in self.tracks_record.keys():
            if len(self.tracks_record[track_id].keys()) < 3 and self.tracks_state[track_id] == True and int(track_id) not in tra_temp:
                ...
                if track_id in self.until_cur_frame_end_tracks.keys():
                    self.until_cur_frame_end_tracks.pop(track_id)
    ...
    def cal_track_feature(self,head_tracks):
        ...
        posis_xy =np.array([det.to_cxy()[:2] for det in head_tracks])
                    ...
                    if (start_deal is True and flag is False) or (start_deal is True and (flag is True and flag1 is True)):
                            ...
                            if int(match_state[row][1]) == -3:
                                ...
                                if len(head_track_sub1_list) > 1:
                                    ...
                                    if row == track_ind:
                                        ...
                                        if '{}'.format(tracks[track_ind]) in self.until_cur_frame_runing_tracks.keys():
                                            if self.tracks_state['{}'.format(tracks[row])] is True:
                                                # 前段轨迹原来未结束，且后段轨迹结束，需要将该前段轨迹结束
                                                frame = self.until_cur_frame_runing_tracks.pop('{}'.format(tracks[track_ind]))
                                                self.add_track_into_until_cur_frame_end_tracks(tracks[track_ind],\
                                                                                               [frame[0],int(tail_part_inds[-1])])

                                                # self.until_cur_frame_end_tracks['{}'.format(tracks[track_ind])] = [frame[0],int(tail_part_inds[-1])]
                                                # 移除后段轨迹
                                                # if "{}".format(tracks[row]) in self.until_cur_frame_end_tracks.keys():
                                                #     self.until_cur_frame_end_tracks.pop('{}'.format(tracks[row]))
                                            # elif self.tracks_state['{}'.format(tracks[row])] is False:
                                                # 前段轨迹原来未结束，且后段轨迹未结束，不用更改
                                                # 移除后段轨迹
                                                # if "{}".format(tracks[row]) in self.until_cur_frame_runing_tracks.keys():
                                                #     self.until_cur_frame_runing_tracks.pop('{}'.format(tracks[row]))
                                        elif '{}'.format(tracks[track_ind]) in self.until_cur_frame_end_tracks.keys():
                                            # 前段轨迹原来已经结束
                                            if self.tracks_state['{}'.format(tracks[row])] is True:
                                                ##后段轨迹已经结束，要更改结束帧
                                                frame = self.until_cur_frame_end_tracks.pop(
                                                    '{}'.format(tracks[track_ind]))
                                                self.add_track_into_until_cur_frame_end_tracks(tracks[track_ind], [
                                                    frame[0], int(tail_part_inds[-1])])

                                                # self.until_cur_frame_end_tracks['{}'.format(tracks[track_ind])] = [
                                                #     frame[0], int(tail_part_inds[-1])]
                                                # 移除后段轨迹
                                                # if "{}".format(tracks[row]) in self.until_cur_frame_end_tracks.keys():
                                                #     self.until_cur_frame_end_tracks.pop('{}'.format(tracks[row]))
                                                ...
                                                frame = self.until_cur_frame_end_tracks.pop(
                                                    '{}'.format(tracks[track_ind]))
                                                self.add_track_into_until_cur_frame_runing_tracks(tracks[track_ind],[
                                                    frame[0]])
                                                # self.until_cur_frame_runing_tracks['{}'.format(tracks[track_ind])] = [
                                                #     frame[0]]
                                                # 移除后段轨迹
                                                # if "{}".format(tracks[row]) in self.until_cur_frame_runing_tracks.keys():
                                                #     self.until_cur_frame_runing_tracks.pop('{}'.format(tracks[row]))
                                    ...
                                    if row == head_track_sub1_list[0]:
                                        ...
                                        if '{}'.format(tracks[matched_head_track_ind]) in self.until_cur_frame_runing_tracks.keys():
                                            if self.tracks_state['{}'.format(tracks[row])] is True:
                                                # 前段轨迹原来未结束，且后段轨迹结束，需要将该轨迹结束
                                                frame = self.until_cur_frame_runing_tracks.pop('{}'.format(tracks[matched_head_track_ind]))
                                                self.add_track_into_until_cur_frame_end_tracks(tracks[matched_head_track_ind],[frame[0],int(tail_part_inds[-1])])

                                                # self.until_cur_frame_end_tracks['{}'.format(tracks[matched_head_track_ind])] = \
                                                #     [frame[0],int(tail_part_inds[-1])]
                                                # 移除后段轨迹
                                                # if "{}".format(tracks[row]) in self.until_cur_frame_end_tracks.keys():
                                                #     self.until_cur_frame_end_tracks.pop('{}'.format(tracks[row]))
                                            # elif self.tracks_state['{}'.format(tracks[row])] is False:
                                                ...
                                                # if "{}".format(tracks[row]) in self.until_cur_frame_runing_tracks.keys():
                                                #     self.until_cur_frame_runing_tracks.pop('{}'.format(tracks[row]))
                                        elif '{}'.format(tracks[matched_head_track_ind]) in self.until_cur_frame_end_tracks.keys():
                                            # 前段轨迹原来已经结束
                                            if self.tracks_state['{}'.format(tracks[row])] is True:
                                                ##后段轨迹已经结束，要更改结束帧
                                                frame = self.until_cur_frame_end_tracks.pop(
                                                    '{}'.format(tracks[matched_head_track_ind]))
                                                self.add_track_into_until_cur_frame_end_tracks(tracks[matched_head_track_ind],\
                                                                                               [frame[0], int(tail_part_inds[-1])])
                                                ...
                                                frame = self.until_cur_frame_end_tracks.pop(
                                                    '{}'.format(tracks[matched_head_track_ind]))
                                                self.add_track_into_until_cur_frame_runing_tracks(tracks[matched_head_track_ind],\
                                                                                                  [frame[0]])
                                                # self.until_cur_frame_runing_tracks['{}'.format(tracks[matched_head_track_ind])] = [
                                                #     frame[0]]
                                                # 移除后段轨迹
                                                # if "{}".format(tracks[row]) in self.until_cur_frame_runing_tracks.keys():
                                                #     self.until_cur_frame_runing_tracks.pop('{}'.format(tracks[row]))
                                    ...
                                    tail_track1 = self.tracks_record['{}'.format(tracks[row])]
                                    a_temp_track_id = tracks[row]
                                    tail_track_sorted_index1 = sorted(tail_track1.keys(),key=lambda k: int(k))
                                    # 取出当前检测在轨迹中的索引位置
                                    tail_detection_posi_index1 = tail_track_sorted_index1.index(frame_id)
                                    tail_part_inds = tail_track_sorted_index1[tail_detection_posi_index1:]
                                    head_temp = {}
                                    ...
                                    temp_track_state['{}'.format(self._next_id)] = self.tracks_state[
                                        '{}'.format(tracks[row])]
                                    # 断开后的轨迹不应该在self.until_cur_frame_runing_tracks、self.until_cur_frame_end_tracks
                                    if self.tracks_state['{}'.format(tracks[row])] is True:
                                        # 结束了，放入self.until_cur_frame_end_tracks
                                        self.add_track_into_until_cur_frame_end_tracks(self._next_id,[int(tail_part_inds[0]),int(tail_part_inds[-1])])

                                        # self.until_cur_frame_end_tracks['{}'.format(self._next_id)] = [tail_part_inds[0],tail_part_inds[-1]]
                                        # 移除后段轨迹
                                        # if "{}".format(tracks[row]) in self.until_cur_frame_end_tracks.keys():
                                        #     self.until_cur_frame_end_tracks.pop('{}'.format(tracks[row]))
                                    else:
                                        # 未结束，放入self.until_cur_frame_runing_tracks
                                        self.add_track_into_until_cur_frame_runing_tracks(self._next_id,[int(tail_part_inds[0])])
                                        # self.until_cur_frame_runing_tracks['{}'.format(self._next_id)] = [tail_part_inds[0]]
                                        # 移除后段轨迹
                                        # if "{}".format(tracks[row]) in self.until_cur_frame_runing_tracks.keys():
                                        #     self.until_cur_frame_runing_tracks.pop('{}'.format(tracks[row]))
                                    # 将后段轨迹与前段轨迹合并
                        ...
                        for row in range(match_state.shape[0]):
                            if int(match_state[row][0]) == row:
                                ...
                                # 判断前段tracks[row]、后段轨迹tracks[int(match_state[row][0])]的状态
                                if '{}'.format(tracks[row]) in self.until_cur_frame_runing_tracks.keys():
                                    # 前段轨迹未结束
                                    frae = self.until_cur_frame_runing_tracks.pop('{}'.format(tracks[row]))
                                    if self.tracks_state['{}'.format(tracks[int(match_state[row][0])])] is False:
                                        # 后段轨迹未结束
                                        # 前段轨迹不变
                                        self.add_track_into_until_cur_frame_runing_tracks(tracks[row],frae)
                                        # self.until_cur_frame_runing_tracks['{}'.format(tracks[row])] = frae
                                        # 移除后段轨迹,前面先从后段轨迹开始处理，此处应该不处理
                                        # if '{}'.format(tracks[int(match_state[row][0])]) in self.until_cur_frame_runing_tracks.keys():
                                        #     self.until_cur_frame_runing_tracks.pop('{}'.format(tracks[int(match_state[row][0])]))
                                    elif self.tracks_state['{}'.format(tracks[int(match_state[row][0])])] is True:
                                        # 后段轨迹结束
                                        # 更新前段轨迹,使之结束
                                        self.add_track_into_until_cur_frame_end_tracks(tracks[row],[frae[0],int(tail_part_inds[-1])])

                                        # self.until_cur_frame_end_tracks['{}'.format(tracks[row])] = [frae[0],tail_part_inds[-1]]
                                        # 移除后段轨迹
                                        # if '{}'.format(tracks[int(match_state[row][0])]) in self.until_cur_frame_end_tracks.keys():
                                        #     self.until_cur_frame_end_tracks.pop('{}'.format(tracks[int(match_state[row][0])]))
                                elif '{}'.format(tracks[row]) in self.until_cur_frame_end_tracks.keys():
                                    # 前段轨迹结束
                                    frae = self.until_cur_frame_end_tracks.pop('{}'.format(tracks[row]))
                                    if self.tracks_state['{}'.format(tracks[int(match_state[row][0])])] is True:
                                        #后段轨迹结束
                                        self.add_track_into_until_cur_frame_end_tracks(tracks[row],[frae[0],int(tail_part_inds[-1])])
                                        ...
                                        # 移除后段轨迹
                                        # if '{}'.format(tracks[int(match_state[row][0])]) in self.until_cur_frame_end_tracks.keys():
                                        #     self.until_cur_frame_end_tracks.pop(
                                        #         '{}'.format(tracks[int(match_state[row][0])]))
                                    elif self.tracks_state['{}'.format(tracks[int(match_state[row][0])])] is False:
                                        #后段轨迹未结束
                                        self.add_track_into_until_cur_frame_runing_tracks(tracks[row],[frae[0]])
                                        # self.until_cur_frame_runing_tracks["{}".format(tracks[row])] = [frae[0]]
                                        # 移除后段轨迹
                                        # if '{}'.format(tracks[int(
                                        #         match_state[row][0])]) in self.until_cur_frame_runing_tracks.keys():
                                        #     self.until_cur_frame_runing_tracks.pop(
                                        #         '{}'.format(tracks[int(match_state[row][0])]))
                                ...
                                if self.tracks_state['{}'.format(tracks[row])] is True:
                                    #本来就是结束状态，该轨迹应该在self.until_cur_frame_end_tracks中
                                    ttt = [self.until_cur_frame_end_tracks["{}".format(tracks[row])][0],int(head_part_inds[-1])]
                                    self.add_track_into_until_cur_frame_end_tracks(tracks[row],ttt)

                                    # self.until_cur_frame_end_tracks["{}".format(tracks[row])][1] = int(head_part_inds[-1])
                                else:
                                    # 原来是未结束状态，该轨迹应该在self.until_cur_frame_runing_tracks中
                                    # 在此处结束，加入self.until_cur_frame_end_tracks
                                    start_frame = self.until_cur_frame_runing_tracks.pop("{}".format(tracks[row]))
                                    print("head_part",start_frame)
                                    self.add_track_into_until_cur_frame_end_tracks(tracks[row],[start_frame[0], int(head_part_inds[-1])])

                                    # self.until_cur_frame_end_tracks["{}".format(tracks[row])] = [start_frame[0], int(head_part_inds[-1])]
                                # start_frame = self.until_cur_frame_runing_tracks.pop("{}".format(tra.track_id))
                                # self.until_cur_frame_end_tracks["{}".format(tra.track_id)] = [start_frame[0], frame_idx]
